...
<head>
    ...
    <script>
        ...
        function goPage(page) {
            url = "${basePath}/overtimeAndOfftimeRecord/list.do";
            var key = $("#key").val();
           /* if(key!=null && key!=""){
             url+="&key="+key;
             }*/
            $("body").load(url,{"page":page,"key":key});
        }
        //删除的方法
...
<div class="widget-main">
    ...
    <div class="input-group">
        <h4 style="float: left;font-weight: bolder">选项:&nbsp;&nbsp;&nbsp;</h4>
              <select id="key" name="key" onchange="goPage(1)" style="display: block;width: 100px;float: left;margin-top: 5px">
                  <option value="0" <c:if test="${key==0}">selected</c:if>>加班记录</option>
                  <option value="1" <c:if test="${key==1}">selected</c:if>>调休记录</option>
              </select>
            </div>
        </div>
    </div>
    ...
    <table class="table table-striped table-bordered table-hover" style="width: 1200px;">
        ...
        <c:forEach items="${offtimeRecords}" var="offtimeRecord" varStatus="status">
            <tr align="center">
                ...
                <td>
                    <c:choose>
                        <c:when test="${offtimeRecord.offtimeDay == 1}">1天</c:when>
                        <c:otherwise>0.5天</c:otherwise>
                    </c:choose>
                </td>
                <td>
